        ...
        for counter, r in enumerate(recordings):
            try:
                for N in repetition:
                    ...
                    file_name_imu = '{}/{}_{}_{}.csv'.format(P, SCENARIO[r], P, r)
                    file_name_imu_attr = '{}/{}_{}_{}_labels.csv'.format(P, SCENARIO[r], P, r)
                    file_name_mocap_data = "{}/{}_{}_{}_{}_{}_norm_data.csv".format(P, SCENARIO[r], P, r,
                                                                                    annotator_file, N)
                    file_name_mocap_attr = "{}/{}_{}_{}_{}_{}_labels.csv".format(P, SCENARIO[r], P, r,
                                                                                 annotator_file, N)

                    print("\n{}\n".format(file_name_imu))

                    if not os.path.exists(dir_dataset + file_name_mocap_data):
                        print("1 - No file in {}".format(dir_dataset + file_name_mocap_data))
                        continue

                    if not os.path.exists(dir_dataset + file_name_mocap_attr):
                        print("1 - No file in {}".format(dir_dataset + file_name_mocap_attr))
                        continue

                    if not os.path.exists(dataset_path_imu_sequences + file_name_imu):
                        print("1 - No file in {}".format(dataset_path_imu_sequences + file_name_imu))
                        continue

                    try:
                        data = csv_reader.reader_labels(dir_dataset + file_name_mocap_attr)
                        labels_imus = data
                    except:
...
def create_dataset(identity_bool = False):
    ...
    base_directory = '/data/sawasthi/LaraMM/'
