...
def configuration(dataset_idx, network_idx, output_idx, usage_modus_idx=0, dataset_fine_tuning_idx=0,
    ...
    configuration = {'dataset': dataset[dataset_idx],
                     ...
                     'model_path': '/data/sawasthi/Opportunity/model/network_Penn_ges_cnn_acc_c1_50.pt'}
            ...
            for arch in networks_arc:
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        ...
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_ntu_loc_cnn_acc_c1_50.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
                ...
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        config = configuration(dataset_idx=1, network_idx=2, output_idx=0, usage_modus_idx=5,
                                               dataset_fine_tuning_idx=ft, learning_rates_idx=1, name_counter=0,
                                               freeze=0, proportions_id = 2, gpudevice = "0")
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_penn_ges_cnn_acc_c1_50.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
