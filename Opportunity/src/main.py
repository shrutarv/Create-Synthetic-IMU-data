...
def configuration(dataset_idx, network_idx, output_idx, usage_modus_idx=0, dataset_fine_tuning_idx=0,
    ...
    epochs = {'locomotion' : {'cnn' : {'softmax' : 94, 'attribute': 5},
                              'lstm' : {'softmax' : 10, 'attribute': 5},
                              'cnn_imu' : {'softmax' : 70, 'attribute': 5}},
              'gesture' : {'cnn' : {'softmax' : 104, 'attribute': 5},
              ...
              'pamap2' : {'cnn' : {'softmax' : 70, 'attribute': 32},
    ...
    if usage_modus[usage_modus_idx] == 'train':
        ...
        folder_exp_base_fine_tuning = '/data/sawasthi/Pamap2/model/network_pamap_cnn_30_nf.pth' #model_acc_up4.pth #model_up1_3a.pt
        '''
        ...
        folder_exp = '/data/sawasthi/PAMAP2/model/'
        folder_exp_base_fine_tuning = '/data/sawasthi/CAD60/model/model_acc_cnn_up3_5sec_tf.pth' #model_acc_up4.pth #model_up1_3a.pt
        '''
        folder_exp = '/data2/fmoya/HAR/pytorch/' + dataset[dataset_idx] + '/' + \
                     network[network_idx] + '/' + output[output_idx] + '/' + fully_convolutional + \
    ...
    proportions = [0.1, 0.2, 0.5, 0.75, 1.0]
    ...
    configuration = {'dataset': dataset[dataset_idx],
                     ...
                     'model_path': '/data/sawasthi/Opportunity/model/network_cad_pamap2_cnn_up1_5s.pt'}
                ...
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        ...
                        setup_experiment_logger(logging_level=logging.DEBUG,
                                                filename=config['folder_exp'] + "logger_cad_PAMAP2_cnn.txt")
                ...
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        config = configuration(dataset_idx=0, network_idx=0, output_idx=0, usage_modus_idx=5,
                                               dataset_fine_tuning_idx=ft, learning_rates_idx=1, name_counter=0,
                                           freeze=0, proportions_id = pp, gpudevice = "0")
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_cad_loc_cnn_acc_c1.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
            ...
            for arch in networks_arc:
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        ...
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_ntu_ges_cnn_acc_c1.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
