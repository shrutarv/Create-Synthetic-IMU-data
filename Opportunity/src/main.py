...
def configuration(dataset_idx, network_idx, output_idx, usage_modus_idx=0, dataset_fine_tuning_idx=0,
    ...
    if usage_modus[usage_modus_idx] == 'train':
        ...
        folder_exp_base_fine_tuning = '/data/sawasthi/Opportunity/model/network_loc_ci_ft.pth' #model_acc_up4.pth #model_up1_3a.pt
        '''
        ...
        folder_exp_base_fine_tuning = '/data/sawasthi/JHMDB/model/model_acc_ci_up4_tf.pth' #model_acc_up4.pth #model_up1_3a.pt
        '''
        folder_exp = '/data2/fmoya/HAR/pytorch/' + dataset[dataset_idx] + '/' + \
                     network[network_idx] + '/' + output[output_idx] + '/' + fully_convolutional + \
    ...
    configuration = {'dataset': dataset[dataset_idx],
                     ...
                     'model_path': '/data/sawasthi/Opportunity/model/network_pamap_ci_acc.pt'}
                ...
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        config = configuration(dataset_idx=3, network_idx=2, output_idx=0, usage_modus_idx=0,
                                               ...
                                               freeze=0, proportions_id=1, gpudevice="1")
                        setup_experiment_logger(logging_level=logging.DEBUG,
                                                filename=config['folder_exp'] + "logger_PAMAP2_ci_acc_nf.txt")
            ...
            for arch in networks_arc:
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        ...
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_loc_ci_10.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
            ...
            for arch in networks_arc:
                for fopt in frezze_opts:
                    for pp in proportions_opts:
                        ...
                        setup_experiment_logger(logging_level=logging.DEBUG, filename= config['folder_exp'] + "logger_ges_ci_50.txt")
                        logging.info('Finished')
                        modus = Modus_Selecter(config)
                        #Starting process
                        modus.net_modus()
