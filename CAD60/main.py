...
def testing(config):
    ...
    torch.load('/data/sawasthi/CAD60/model/model_acc_cnn_up1_tf_test.pth', map_location=torch.device('cpu'))['state_dict']
    model.eval()
    model.to(device)
    loss_test = 0.0
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    for iter in range(iterations):
        ...
        model_path = '/data/sawasthi/CAD60/model/model_acc_cnn_up1_tf_t.pth'
        ...
        #testing(config)
        # Test data    
        path = '/data/sawasthi/data/CAD60/testData_acc_up1/'
        #path = 'S:/MS A&R/4th Sem/Thesis/LaRa/IMU data/IMU data/Windows/'
        #path = "S:/MS A&R/4th Sem/Thesis/LaRa/OMoCap data/Test_data/"
        test_dataset = CustomDataSet(path)
